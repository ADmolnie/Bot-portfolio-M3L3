...
/start - начать писанину
/info - информация о проекте
/new_project - добавить новый проект
/skills - добавляет навык
...
def name_project(message):
    ...
    bot.register_next_step_handler(message, decription_name, data=data)

# ссылка на проект
def decription_name(message, data):
    decription = message.text
    data.append(decription)
...
# если статус не из списка, то выбор идёт ещё раз, затем бот сохраняет проект
...
# выбор проекта, затем выбераем навык
...
# бот присылает все проекты, которые у тебя есть
